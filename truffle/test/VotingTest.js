...
contract("Voting", function (accounts) {
  ...
  context("Add Proposal Phase", function() {
    beforeEach(async function () {
      ...
      it('Test on require: non voter cant propose', async function () {
        await Voting.startProposalsRegistering({from: owner})
        await expectRevert(Voting.addProposal("BadOwner", {from: accounts[4]}),
        "You're not a voter")
      })
      ...
      it("Multiple Proposal pass : concat", async function () {
        ...
        expect(voter2ProposalID.description).to.be.equal("proposalVoter1");
        expect(voter3ProposalID.description).to.be.equal("proposalVoter2");
      })
  ...
  context("Voting Phase", function() {
    ...
    it('Concat : Test on requires: non voter cant propose, voter cant propose nothing, and voter cant vote twice', async function () {
        ...
        await expectRevert(Voting.setVote(1, {from:accounts[4]}),
        "You're not a voter")
  ...
  context("Temporary winningPropId on setVote() function test", function() {
    ...
    beforeEach(async function () {
      ...
      await Voting.addVoter(voter3, {from: owner})
      await Voting.startProposalsRegistering({from: owner})
      await Voting.addProposal("proposal 1", {from: owner})
      await Voting.addProposal("proposal 2", {from: voter1})
      await Voting.addProposal("proposal 2", {from: voter2})
      await Voting.endProposalsRegistering({from: owner})
      await Voting.startVotingSession({from:owner});
    })
            ...
            it("Winning test 1 // VoteCount // WiningProposal = WinningProposal1 (2 votes)", async function() {
                ...
                await Voting.setVote(1,{from:owner});
                await Voting.setVote(1,{from:accounts[1]});
                await Voting.setVote(0,{from:accounts[2]});
                await Voting.endVotingSession({from:owner});
      
                const winningPropId = await Voting.getOneProposal(1);
                expect(winningPropId.description).to.be.equal("proposal 1");
                expect(winningPropId.voteCount).to.be.bignumber.equal(new BN(2));        
            })
  ...
  context("Tallying Phase", function() {
    ...
    beforeEach(async function () {
      ...
      it('Tally pass, test on winning proposal description and vote count', async function () {
        ...
        expect(winningProposal.description).to.equal('voter1Proposal');
        expect(winningProposal.voteCount).to.equal('1');
      })
      it('Tally test, winningPropId = winningProposalId', async function () {
        await Voting.endVotingSession({from: owner})
        await Voting.tallyVotes({from: owner});
        let winningID = await Voting.winningPropId.call();
        let winningProposal = await Voting.getOneProposal(winningID, {from:voter1});
        expect(winningProposal.description).to.equal('voter1Proposal');
        expect(winningProposal.voteCount).to.equal('1');
      })
